//! Error type for the auth-service client SDK.

use thiserror::Error;

/// All errors returned by [`crate::AuthClient`] service methods.
///
/// Three broad categories:
/// - [`AuthClientError::Transport`] — connection / transport-level failures
///   (DNS, TCP, TLS, channel closure, malformed endpoint URI, …).
/// - [`AuthClientError::Status`] — a gRPC call reached the server but was
///   rejected with a non-OK [`tonic::Status`] (e.g. `Unauthenticated`,
///   `NotFound`, `PermissionDenied`).
/// - [`AuthClientError::InvalidResponse`] — the call succeeded but the response
///   could not be interpreted as expected (decoding / shape mismatch).
#[derive(Debug, Error)]
